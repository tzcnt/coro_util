...
// Provides coro_util::qu_spsc_bounded_impl, an async SPSC bounded linearizable
...
// Uses a similar fetch-add slot acquisition scheme to coro_util::channel,
// with these changes:
...
struct qu_spsc_bounded_default_config {
  ...
  /// The SPSC queue is not padded by default to improve cache coherency for the
  /// single producer.
...
template <
  ...
  typename Config = coro_util::qu_spsc_bounded_default_config>
class qu_spsc_bounded_impl {
  ...
  static inline constexpr uintptr_t CLOSED_BIT = static_cast<size_t>(1) << 1;
  ...
  // We assert that nullptr == 0 (in shared.hpp), and that the lower 2 bits of a
  ...
  // Use explicit padding instead of alignas(CORO_UTIL_CACHE_LINE_SIZE) to prevent false
  // sharing, because this may be created inside of a coroutine frame, and compilers don't
  // automatically respect overaligned allocations inside of coroutine frames at the
  // moment.
  ...
  char pad3[CORO_UTIL_CACHE_LINE_SIZE - sizeof(size_t)];
...
public:
  ...
  class try_pull_zc_scope {
    friend qu_spsc_bounded_impl;
    qu_spsc_bounded_impl* queue;
    element* elem;
    size_t idx;
    coro_util::qu_err err;

    try_pull_zc_scope(qu_spsc_bounded_impl* Queue, element* Elem, size_t Idx) noexcept
  ...
  public:
    ...
    try_pull_zc_scope() noexcept
        : queue{nullptr}, elem{nullptr}, idx{0}, err{coro_util::qu_err::EMPTY} {}
  ...
  class pull_zc_scope {
    ...
    pull_zc_scope(qu_spsc_bounded_impl* Queue, element* Elem, size_t Idx) noexcept
  ...
  public:
    ...
    /// Destroys the object in the queue storage and releases the queue slot.
    CORO_UTIL_FORCE_INLINE ~pull_zc_scope() {
  ...
  /// Constructs a qu_spsc_bounded_impl with the given capacity.
  explicit qu_spsc_bounded_impl(size_t Capacity) noexcept
    ...
    assert(
      Capacity < (static_cast<size_t>(1) << (CORO_UTIL_PLATFORM_BITS - 1)) &&
...
private:
  ...
  static void resume_producer(producer_base* Prod) noexcept {
    ContinuationPolicy::resume(Prod->continuation_state, Prod->continuation);
  }
  ...
  static void resume_consumer_inline(consumer_base* Cons) noexcept {
    ContinuationPolicy::resume_inline(Cons->continuation_state, Cons->continuation);
  }
...
public:
  ...
  template <typename... Args>
  [[nodiscard(
    "You must co_await push(). The value will not be enqueued until "
    "co_await."
  )]]
...
public:
  ...
  template <typename... Args> class aw_push final {
    friend qu_spsc_bounded_impl<ContinuationPolicy, T, Config>;
    ...
    aw_push(qu_spsc_bounded_impl& Queue, Args&&... ConstructArgs) noexcept
    ...
    struct aw_push_impl final {
      ...
      aw_push_impl(aw_push& Parent) noexcept
          : base{{}, nullptr}, queue(Parent.queue), args(Parent.args), elem(nullptr),
            idx(0) {}
      ...
      void await_resume() noexcept {
        ...
        if (cons != nullptr) {
          queue.resume_consumer(cons);
        }
      }
    };

  public:
    using queue_type = qu_spsc_bounded_impl;

    aw_push_impl operator co_await() && noexcept { return aw_push_impl(*this); }
  };

  /// Returns `void` when awaited.
  template <typename It> class aw_push_bulk final {
    friend qu_spsc_bounded_impl<ContinuationPolicy, T, Config>;
    ...
    aw_push_bulk(qu_spsc_bounded_impl& Queue, It Items, size_t Count) noexcept
    ...
    struct aw_push_bulk_impl final {
      ...
      aw_push_bulk_impl(aw_push_bulk& Parent) noexcept
          : base{{}, nullptr}, queue(Parent.queue), items(Parent.items),
            count(Parent.count), startIdx(0), lastElem(nullptr) {}
  ...
  class aw_pull final {
    ...
    aw_pull(qu_spsc_bounded_impl& Queue) noexcept : queue(Queue) {}
    ...
    struct aw_pull_impl final {
      ...
      aw_pull_impl(aw_pull& Parent) noexcept
          : base{{}, nullptr, nullptr}, queue{Parent.queue}, idx{0} {}
      ...
      pull_zc_scope await_resume() noexcept {
  ...
  ///   - qu_err::CLOSED - the queue has been closed and drained
  ///
  /// The returned scope's `has_value()` / `operator bool()` returns true if a
  ...
  try_pull_zc_scope try_pull() {
    ...
    if (s == CLOSED_BIT) {
      return try_pull_zc_scope(coro_util::qu_err::CLOSED);
    }
    return try_pull_zc_scope(coro_util::qu_err::EMPTY);
  }
  ...
  qu_spsc_bounded_impl(const qu_spsc_bounded_impl&) = delete;
  qu_spsc_bounded_impl& operator=(const qu_spsc_bounded_impl&) = delete;
  qu_spsc_bounded_impl(qu_spsc_bounded_impl&&) = delete;
  qu_spsc_bounded_impl& operator=(qu_spsc_bounded_impl&&) = delete;
};
