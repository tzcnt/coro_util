...
// coro_util::semaphore-based version, but it had the same constraints:
...
struct qu_mpsc_bounded_default_config {
  ...
  /// If true, queue elements will be padded up to the next increment of
  /// CORO_UTIL_CACHE_LINE_SIZE bytes. This reduces false sharing between
  /// neighboring elements. If false, no padding will be applied.
...
class qu_mpsc_bounded_impl {
  ...
  static inline constexpr uintptr_t PROD_BIT = static_cast<size_t>(1);
  static inline constexpr uintptr_t DATA_BIT = static_cast<size_t>(1) << 1;
  static inline constexpr uintptr_t CONS_BIT = static_cast<size_t>(1) << 2;
...
private:
  ...
  element* get_write_ticket() noexcept {
    ...
    if (closed.load(std::memory_order_relaxed)) [[unlikely]] {
      ...
      if (!circular_less_than(idx, write_closed_at.load(std::memory_order_acquire))) {
        return nullptr;
      }
    }
  ...
  producer_wait_result producer_try_claim(element* Elem) noexcept {
    ...
    while ((cur & ~CONS_BIT) == 0) {
      if (Elem->flags.compare_exchange_weak(
            cur, cur | PROD_BIT, std::memory_order_acq_rel, std::memory_order_acquire
          )) {
  ...
  producer_wait_result producer_park(element* Elem, producer_base* MyNode) noexcept {
    uintptr_t cur = Elem->flags.load(std::memory_order_acquire);
    while (true) {
      if ((cur & ~CONS_BIT) == 0) {
...
private:
  ...
  consumer_base* close_get_waiting_consumer() noexcept {
    ...
    while (true) {
      ...
      if ((cur & CONS_BIT) != 0) {
        size_t wait_idx = elem->waiting_consumer_idx.load(std::memory_order_relaxed);
        ...
        if (elem->flags.compare_exchange_weak(
              cur, CLOSE_SENTINEL, std::memory_order_acq_rel, std::memory_order_acquire
            )) {
          return nullptr;
        }
      }
  ...
  template <typename... Args> class aw_push final {
    ...
    struct aw_push_impl final {
      ...
      aw_push_impl(aw_push& Parent) noexcept
          : base{nullptr, {}, nullptr}, queue(Parent.queue), args(Parent.args),
            elem(nullptr), closed_before_enqueue(false) {}
      ...
      bool await_resume() noexcept {
        ...
        // value category.
        std::apply(
          [this](auto&&... a) { elem->data.emplace(static_cast<decltype(a)&&>(a)...); },
          std::move(args)
        );
      ...
      bool await_ready() noexcept {
        while (true) {
          ...
          if (queue.closed.load(std::memory_order_relaxed)) {
            ...
            size_t cutoff = queue.write_closed_at.load(std::memory_order_acquire);
          ...
          while (true) {
            ...
            if (has_close_sentinel(f)) {
              ...
              if (elem->waiting_consumer.compare_exchange_strong(
                ...
                size_t cutoff = q->write_closed_at.load(std::memory_order_acquire);
                if (!circular_less_than(idx, cutoff)) {
            ...
            assert((f & CONS_BIT) == 0);
            if (elem->flags.compare_exchange_weak(
                  f, f | CONS_BIT, std::memory_order_acq_rel, std::memory_order_acquire
                )) {
              return true;
            }
          }
