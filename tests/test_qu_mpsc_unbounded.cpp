...
template <bool Padding> struct q_config : coro_util::qu_mpsc_unbounded_default_config {
...
template <bool ElementPadding, typename Executor> void do_q_test(Executor& Exec) {
  test_async_main(Exec, []() -> tmc::task<void> {
    {
      ...
      auto q = coro_util::qu_mpsc_unbounded<size_t, q_config<ElementPadding>>{};
    ...
    }
    {
      ...
      {
        auto q = coro_util::qu_mpsc_unbounded<
          mpsc_destructor_counter, q_config<ElementPadding>>{};
...
TEST_F(CATEGORY, post_bulk_none) {
  ...
  test_async_main(ex, []() -> tmc::task<void> {
    auto q = coro_util::qu_mpsc_unbounded<size_t, q_config<true>>{};
    size_t i = 0;
    for (; i < 4; ++i) {
      q.post_bulk(&i, 0);
...
TEST_F(CATEGORY, no_suspend_try_pull_only) {
  test_async_main(ex(), []() -> tmc::task<void> {
    using qerr = coro_util::qu_err;
    auto q = coro_util::qu_mpsc_unbounded<size_t, q_config_no_suspend>{};
...
TEST_F(CATEGORY, try_pull_zc_scope_move) {
  test_async_main(ex(), []() -> tmc::task<void> {
    // Move-construct.
    {
      std::atomic<size_t> count{0};
      {
        auto q = coro_util::qu_mpsc_unbounded<mpsc_destructor_counter, q_config<true>>{};
        q.post(mpsc_destructor_counter{&count});
        q.post(mpsc_destructor_counter{&count});
        q.post(mpsc_destructor_counter{&count});
    ...
    // Move-assign over an empty scope.
    {
      std::atomic<size_t> count{0};
      {
        using qu = coro_util::qu_mpsc_unbounded<mpsc_destructor_counter, q_config<true>>;
        qu q{};
        q.post(mpsc_destructor_counter{&count});
        q.post(mpsc_destructor_counter{&count});
...
TEST_F(CATEGORY, non_movable_type) {
  test_async_main(ex(), []() -> tmc::task<void> {
    auto q = coro_util::qu_mpsc_unbounded<non_movable, q_config<true>>{};
...
TEST_F(CATEGORY, post_variadic_args) {
  test_async_main(ex(), []() -> tmc::task<void> {
    auto q = coro_util::qu_mpsc_unbounded<mpsc_multi_arg, q_config<true>>{};
